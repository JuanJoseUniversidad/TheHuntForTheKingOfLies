...
public:
	/**
	* Enables the character (input, animation etc)
	*/
	void EnableCharacter();

	/**
	 * Disables the character (input, animation etc)
	 */
	void DisableCharacter();

	/**
	 * Gets the last ragdoll velocity
	 * @return Returns the FVector of velocity
	 */
	FVector GetLastRagdollVelocity();

	/**
	 * Sets the ragdoll mode
	 * @param bNewRagdollMode - The new ragdoll mode to enter
	 */
	UFUNCTION(BlueprintCallable, Category="ALS|Ragdoll System")
	void SetCanRagdollMode(bool bNewRagdollMode);

public:
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "ALS | Properties | Fall")
		float TimeInAirToPlaySound = 2.0;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "ALS | Properties | Fall")
		USoundCue* FallSoundCue;

	UAudioComponent* AudioFallingRefence;

	bool bBlockActions = false;
	FVector PositionFalling;

protected:
	bool bCanChangeMovementState = true;
...
private:
	...
	FTimerHandle TimerHandleFallRagdoll;
};
