...
struct FALSMovementSettings
{
	...
	UPROPERTY(EditAnywhere, Category = "Movement Settings")
	float RandomSpeedReductionFactor = 0.0f;
	...
	float GetSpeedForGait(const EALSGait Gait) const
	{
		...
		case EALSGait::Running:
			return FMath::Abs(RunSpeed - FMath::RandRange(0.0f, RandomSpeedReductionFactor));
		case EALSGait::Sprinting:
			return FMath::Abs(SprintSpeed - FMath::RandRange(0.0f, RandomSpeedReductionFactor));
		case EALSGait::Walking:
			return FMath::Abs(WalkSpeed - FMath::RandRange(0.0f, RandomSpeedReductionFactor));
		default:
			return FMath::Abs(RunSpeed - FMath::RandRange(0.0f, RandomSpeedReductionFactor));
		}
	}
...
struct FALSHitFX : public FTableRowBase
{
	...
	TSoftObjectPtr<UNiagaraSystem> NiagaraSystemDefault;

	UPROPERTY(EditAnywhere, Category = "Niagara")
	TSoftObjectPtr<UNiagaraSystem> NiagaraSystemStep;
...
USTRUCT(BlueprintType)
struct FALSBallisticHitFX : public FTableRowBase
{
	GENERATED_BODY()
	...
	UPROPERTY(EditAnywhere, Category = "Decal")
		TArray<TSoftObjectPtr<UMaterialInterface>> DecalsMaterial;
	...
	UPROPERTY(EditAnywhere, Category = "Cascade")
		FVector CascadeLocationOffset;

	UPROPERTY(EditAnywhere, Category = "Cascade")
		FRotator CascadeRotationOffset;
};


USTRUCT(BlueprintType)
struct FALSFallImpactFX : public FTableRowBase
{
	GENERATED_BODY()
