...
void AALSBaseCharacter::BeginPlay()
{
	...
	//Avoids to insta die if enters on ragdoll without jumping one time
	PositionFalling = GetActorLocation();
}
...
void AALSBaseCharacter::OnMovementModeChanged(EMovementMode PrevMovementMode, uint8 PreviousCustomMode)
{
		...
		GetCharacterMovement()->MovementMode == MOVE_NavWalking)
	{
		...
		//Stops the timer to ragdoll when its in the air
		GetWorld()->GetTimerManager().ClearTimer(TimerHandleFallRagdoll);
	}
	else if (GetCharacterMovement()->MovementMode == MOVE_Falling)
	{
		...
		/* EDITED */
		//Sets the initial position of the falling
		PositionFalling = GetActorLocation();
		// When its in the air n seconds a scream sound and start the ragdoll state
		GetWorld()->GetTimerManager().SetTimer(TimerHandleFallRagdoll, [&]() {
			if (GetMovementState() == EALSMovementState::InAir) {
				//Play sound
				AudioFallingRefence = UGameplayStatics::SpawnSoundAttached(FallSoundCue,GetMesh());
				//AudioFallingRefence->bAutoDestroy = true;

				//Starts the ragdoll state
				ReplicatedRagdollStart();
			}
		GetWorld()->GetTimerManager().ClearTimer(TimerHandleFallRagdoll);
		}, TimeInAirToPlaySound, false);
	}
}

void AALSBaseCharacter::OnMovementStateChanged(const EALSMovementState PreviousState)
{
	if (bCanChangeMovementState == true) {//-Edited
...
// EDITED
void AALSBaseCharacter::EnableCharacter() {
...
// EDITED
void AALSBaseCharacter::DisableCharacter() {
...
// EDITED
FVector AALSBaseCharacter::GetLastRagdollVelocity() {
	return LastRagdollVelocity;
}

void AALSBaseCharacter::SetCanRagdollMode(bool bNewRagdollMode) {
	bCanRagdollMode = bNewRagdollMode;
}
