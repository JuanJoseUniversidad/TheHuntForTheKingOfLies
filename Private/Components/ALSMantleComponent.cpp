...
void UALSMantleComponent::BeginPlay()
{
	...
	if (GetOwner())
	{
		...
		OwnerPlayerCharacter = Cast<AALSCharacter>(GetOwner());//--
                                        ...
                                        FActorComponentTickFunction* ThisTickFunction)
{
	...
	//--
	if (OwnerCharacter && OwnerCharacter->GetMovementState() == EALSMovementState::InAir && OwnerPlayerCharacter && OwnerPlayerCharacter->bBlockActions == false)
	{
                                      ...
                                      EALSMantleType MantleType)
{
	...
	//-- To avoid climb on physical objects and get launched
	// Disable the physics
	if (MantleLedgeLS.Component->IsSimulatingPhysics()) {
		bPhysicsActor = true;
		MantleLedgeLS.Component->SetSimulatePhysics(false);
	}
	else {
		bPhysicsActor = false;
	}
...
void UALSMantleComponent::MantleEnd()
{
	...
	//-- To avoid climb on physical objects and get launched
	// Set the physics to normal
	if (bPhysicsActor == true) {
		MantleLedgeLS.Component->SetSimulatePhysics(true);
	}
